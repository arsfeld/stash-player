...
- Release tagging (`v*`) triggers `.github/workflows/flatpak.yml`, which attaches `stash-player.flatpak` to the GitHub release (replaced by `publish-repo`'s re-export on stable tags); its cache key is `hashFiles('apps/flutter/pubspec.lock', 'build-aux/dev.arsfeld.stash-player.yml')`.
